...
class khachhangTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $nCustomers = 500; //khai bao so khach hang
        $minFemales = 100; // so nu toi thieu
        $maxFemales = 350; // so nu toi da
        // sinh ngau nhien so khach nu va nam
        ...
        for ($i=1; $i <= $nCustomers; $i++) {
            ...
            array_push($list, [
                ...
                'kh_email'     => $email,
                'kh_ngaySinh'  => $now = new Carbon($birthdate["birthdate"], 'Asia/Ho_Chi_Minh'),
                'kh_diaChi'    => $address,
                'kh_dienThoai' => $phone,
